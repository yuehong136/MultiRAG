...
class MessageParam(ComponentParamBase):
    ...
    def __init__(self):
        ...
        self.memory_ids = []  # Memory IDs to save conversation
        self.outputs = {
            "content": {
                "type": "str"
            }
...
class Message(ComponentBase):
    ...
    async def _consume_async_gen(self, agen):
        """Helper to consume an async generator and return the accumulated string."""
        buf = ""
        async for t in agen:
            buf += t
        return buf
    ...
    def _convert_content(self, content):
        ...
        try:
            if self._param.output_format in {"markdown", "html"}:
                ...
                logging.info(
                    f"XLSX Parser: Content length={len(content) if content else 0}, first 500 chars: {content[:500] if content else 'None'}")
                ...
                if isinstance(content, str):
                    ...
                    logging.info(
                        f"XLSX Parser: Total lines={len(lines)}, lines starting with '|': {sum(1 for line in lines if line.strip().startswith('|'))}")
                    current_table_lines = []
    ...
    async def _save_to_memory(self, content):
        ...
        error_text = "Some messages failed to add. " + " ".join(
            [f"Add to memory {r['memory_id']} failed, detail: {r['msg']}" for r in res if not r["success"]]
        )
