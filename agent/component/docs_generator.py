...
class PDFGenerator(Message, ABC):
        ...
        try:
            ...
            if content and self._canvas.is_reff(content.strip()):
                ...
                for match in matches:
                    try:
                        ...
                        print(f"Error resolving variable {match}: {e!s}")
                        content = content.replace("{" + match + "}", f"[ERROR: {e!s}]")
            ...
            if content:
                try:
                    ...
                    print(f"Error processing content with get_kwargs: {e!s}")
            ...
            if title and self._canvas.is_reff(title):
                try:
                    ...
                    print(f"Error processing title variables: {e!s}")
            ...
            if subtitle and self._canvas.is_reff(subtitle):
                try:
                    ...
                    print(f"Error processing subtitle variables: {e!s}")
            ...
            try:
                ...
                error_msg = f"Error in _generate_pdf: {e!s}\n{traceback.format_exc()}"
                print(error_msg)
                self.set_output("success", False)
                self.set_output("_ERROR", f"PDF generation failed: {e!s}")
                raise

        except Exception as e:
            error_msg = f"Error in PDFGenerator._invoke: {e!s}\n{traceback.format_exc()}"
            print(error_msg)
            self.set_output("success", False)
            self.set_output("_ERROR", f"PDF generation failed: {e!s}")
            raise
    ...
    def _generate_pdf(self, content: str, title: str = "", subtitle: str = "") -> tuple[str, str]:
        ...
        try:
            ...
            error_msg = f"Error generating PDF: {e!s}\n{traceback.format_exc()}"
            print(error_msg)
    ...
    def _create_table(self, table_lines: list[str]) -> list | None:
    ...
    def _generate_docx(self, content: str, title: str = "", subtitle: str = "") -> tuple[str, str]:
        ...
        try:
            ...
            raise Exception(f"DOCX generation failed: {e!s}")
    ...
    def _generate_txt(self, content: str, title: str = "", subtitle: str = "") -> tuple[str, str]:
        ...
        try:
            ...
            raise Exception(f"TXT generation failed: {e!s}")
