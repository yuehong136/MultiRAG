...
这个文件夹包含了MultiRAG的插件机制。

MultiRAG将会从`embedded_plugins`子文件夹中递归加载所有的插件。
...
这些描述信息将被提供给LLM进行调用，和MultiRAG的Web前端用作展示。

- `invoke`方法：这个方法接受LLM生成的参数，并且返回一个`str`对象，其中包含了这个工具的执行结果。
这个工具的所有执行逻辑都应当放到这个方法里。

当你启动MultiRAG时，你会在日志中看见你的插件被加载了：

```
2025-05-15 19:29:08,959 INFO     34670 Recursively importing plugins from path `/some-path/MultiRAG/agent/plugin/embedded_plugins`
...
def get_metadata(cls) -> LLMToolMetadata:
    return {
        # 这个工具的名称，会提供给LLM
        "name": "bad_calculator",
        # 这个工具的展示名称，会提供给MultiRAG的Web前端
        "displayName": "$t:bad_calculator.name",
        # 这个工具的用法描述，会提供给LLM
        "description": "A tool to calculate the sum of two numbers (will give wrong answer)",
        # 这个工具的描述，会提供给MultiRAG的Web前端
...
以`display`开头的字段可以使用一种特殊写法`$t:xxx`，这种写法将使用MultiRAG的国际化机制，从`llmTools`这个分类中获取文字。如果你不使用这种写法，那么前端将会显示此处的原始内容。
