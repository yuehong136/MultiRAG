...
class LLMParams:
    ...
    def from_params_list(cls, params_list: list[dict[str, Any]]) -> "LLMParams":
        """从参数列表创建LLMParams实例"""
        # 创建参数映射
        param_map = {param["name"]: {"value": cls._extract_param_value(param), "type": param.get("input", {}).get("type")} for param in params_list}

        return cls(
            model_name=cls._convert_value(
                param_map.get("modelName", {}).get("value") or param_map.get("modleName", {}).get("value") or "gpt-3.5-turbo",
                param_map.get("modelName", {}).get("type") or param_map.get("modleName", {}).get("type"),
            ),
            temperature=cls._convert_value(param_map.get("temperature", {}).get("value", 0.7), param_map.get("temperature", {}).get("type")),
            top_p=cls._convert_value(param_map.get("topP", {}).get("value", 1.0), param_map.get("topP", {}).get("type")),
            max_tokens=cls._convert_value(param_map.get("maxTokens", {}).get("value"), param_map.get("maxTokens", {}).get("type")),
            frequency_penalty=cls._convert_value(param_map.get("frequencyPenalty", {}).get("value", 0.0), param_map.get("frequencyPenalty", {}).get("type")),
            response_format=cls._convert_value(param_map.get("responseFormat", {}).get("value", 2), param_map.get("responseFormat", {}).get("type")),
            prompt=cls._convert_value(param_map.get("prompt", {}).get("value"), param_map.get("prompt", {}).get("type")),
            system_prompt=cls._convert_value(param_map.get("systemPrompt", {}).get("value"), param_map.get("systemPrompt", {}).get("type")),
            model_type=cls._convert_value(param_map.get("modelType", {}).get("value"), param_map.get("modelType", {}).get("type")),
            generation_diversity=cls._convert_value(param_map.get("generationDiversity", {}).get("value"), param_map.get("generationDiversity", {}).get("type")),
            enable_chat_history=cls._convert_value(param_map.get("enableChatHistory", {}).get("value"), param_map.get("enableChatHistory", {}).get("type")),
        )
    ...
    def _build_history(self, prompt: str, inputs: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        if self.llm_params.enable_chat_history:
            ...
            if isinstance(candidate_history, list):
                history.extend(item for item in candidate_history if isinstance(item, dict) and item.get("role") and item.get("content") is not None)
