    ...
    def validate_selector_branches(self) -> None:
        ...
        for node in self.nodes:
            ...
            if node['type'] == '8':
                ...
                connected_ports = set()
                for edge in self.edges:
                    if (edge['sourceNodeID'] == node_id and
        ...
        for edge in self.edges:
            ...
            if target in selector_targets:
                target_sources[target].append({
                    ...
                    'isSelector': self._is_selector_edge(edge)
                })

        # 对于每个目标节点，检查是否有非选择器节点的入边
        for target, sources in target_sources.items():
            non_selector_sources = [
                source['nodeID'] for source in sources
                if not source['isSelector']
            ]
            ...
            if non_selector_sources:
                ...
                for node in involved_nodes:
                    self.issues.append(ValidationIssue(
                        f"Node {target} 连接到选择器节点，但也有来自非选择器节点的传入边",
                        [node]
                    ))
