...
# MultiRAG Data Migration Script
# Usage: ./migration.sh [-p project_name] [backup|restore] [backup_folder]
#
# This script helps you backup and restore MultiRAG Docker volumes
# including PostgreSQL, MinIO, Redis, and Milvus data.
...
VOLUME_BASES=("postgres_data" "minio_data" "redis_data" "milvus_etcd_data" "milvus_minio_data" "milvus_data")
BACKUP_FILES=("postgres_backup.tar.gz" "minio_backup.tar.gz" "redis_backup.tar.gz" "milvus_etcd_backup.tar.gz" "milvus_minio_backup.tar.gz" "milvus_backup.tar.gz")
...
show_help() {
    echo "MultiRAG Data Migration Tool"
    echo ""
    echo "USAGE:"
    echo "  $0 [-p project_name] <operation> [backup_folder]"
    echo ""
    echo "OPERATIONS:"
    echo "  backup   - Create backup of all MultiRAG data volumes"
    echo "  restore  - Restore MultiRAG data volumes from backup"
    echo "  help     - Show this help message"
    echo ""
    echo "OPTIONS:"
    echo "  -p project_name  - Docker Compose project name (default: '$DEFAULT_PROJECT_NAME')"
    echo "                     Use this when you started MultiRAG with 'docker compose -p <name>'"
    echo ""
    echo "PARAMETERS:"
    ...
    echo "  $0 -p multirag backup              # Backup volumes for project 'multirag'"
    echo "  $0 -p multirag restore my_backup   # Restore volumes for project 'multirag'"
    echo ""
    ...
    echo "  - ${DEFAULT_PROJECT_NAME}_milvus_etcd_data    (Milvus etcd metadata)"
    echo "  - ${DEFAULT_PROJECT_NAME}_milvus_minio_data   (Milvus MinIO storage)"
    echo "  - ${DEFAULT_PROJECT_NAME}_milvus_data         (Milvus vector data)"
    echo ""
    echo "NOTE:"
    echo "  If you started MultiRAG with 'docker compose -p myproject up', the volume"
...
check_containers_using_volumes() {
    ...
    if [ ${#containers_using_volumes[@]} -gt 0 ]; then
        ...
        if [ "$PROJECT_NAME" = "$DEFAULT_PROJECT_NAME" ]; then
            ...
            echo "   docker compose -f docker/docker-compose-base.yml down"
        else
            echo "🛑 SOLUTION: Stop the containers before performing backup/restore operations:"
            echo "   docker compose -p $PROJECT_NAME -f docker/docker-compose-base.yml down"
        fi
        echo ""
...
perform_backup() {
    ...
    echo "🚀 Starting MultiRAG data backup..."
...
perform_restore() {
    ...
    echo "🔄 Starting MultiRAG data restore..."
    echo "📁 Backup folder: $backup_folder"
    echo "🏷️  Project name: $PROJECT_NAME"
    echo ""
    ...
    echo "💡 You can now start your MultiRAG services"
}
