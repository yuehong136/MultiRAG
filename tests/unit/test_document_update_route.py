...
PUT /api/v1/datasets/{id}/documents/{id} 走真实 ``api.apps.app`` 的 HTTP 契约式；
...
def test_update_document_route_has_pure_async_dependency_tree(client, route_dependency_calls):
    ...
    calls = route_dependency_calls(client.app, "PUT", "/api/v1/datasets/{dataset_id}/documents/{document_id}")
