"""Langfuse host 的 SSRF 出网边界（validate_outbound_url + langfuse_app 两端点）。
...
def test_set_api_key_blocks_private_host_before_probe(client, monkeypatch):
    monkeypatch.setattr(_route_module(), "Langfuse", _BombLangfuse)

    resp = client.post("/v1/langfuse/api_key", json=_KEYS)
...
def test_set_api_key_allowlisted_host_passes_boundary(client, monkeypatch):
    ...
    monkeypatch.setattr(module, "get_app_config", lambda: types.SimpleNamespace(observability=types.SimpleNamespace(langfuse_allowed_hosts=["langfuse.internal"])))
    ...
    monkeypatch.setattr(module, "Langfuse", _OkLangfuse)
    ...
    resp = client.post("/v1/langfuse/api_key", json={**_KEYS, "host": "http://langfuse.internal:3000"})
