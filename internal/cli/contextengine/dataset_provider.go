...
func (p *DatasetProvider) searchWithRetrieval(ctx stdctx.Context, opts *SearchOptions) (*Result, error) {
	...
	// NOTE: MultiRAG's RetrievalTestRequest expects the field "kb_ids" (list),
	// whereas RAGFlow's web handler reads "kb_id". We send "kb_ids" to match
	// our Pydantic contract.
...
func (p *DatasetProvider) searchDocuments(ctx stdctx.Context, datasetName string, opts *SearchOptions) (*Result, error) {
	...
	// NOTE: send "kb_ids" to match MultiRAG's RetrievalTestRequest (see searchWithRetrieval).
	payload := map[string]interface{}{
		"kb_ids":   []string{kbID},
		"question": opts.Query,
	}
