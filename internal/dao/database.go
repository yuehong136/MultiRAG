...
func InitDB() error {
	...
	case "mysql", "":
		...
		if err != nil {
			return fmt.Errorf("failed to connect mysql database: %w", err)
		}
	default:
...
func autoMigrateSafely(db *gorm.DB, model interface{}) error {
	...
	// PostgreSQL errors
	// 42P07: duplicate_table, 42701: duplicate_column, 42P16: invalid_table_definition (e.g., column in primary key)
	// 23502: not_null_violation (existing data has NULLs, AutoMigrate can't add NOT NULL constraint)
...
func InitLLMFactory() error {
	configPath := filepath.Join(utility.GetProjectBaseDirectory(), "configs", "llm_factories.json")
...
// IsPostgres returns true if the configured database driver is PostgreSQL
func IsPostgres() bool {
	cfg := server.GetConfig()
	return cfg.Database.Driver == "postgres" || cfg.Database.Driver == "postgresql"
}
