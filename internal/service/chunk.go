...
// buildRetrievalTestResults preserves the local retrieval response mapper for callers that
// still provide engine-shaped chunks. The new retrieval service already emits these names,
// so canonical fields pass through unchanged.
func buildRetrievalTestResults(chunks []map[string]interface{}) []map[string]interface{} {
	results := make([]map[string]interface{}, 0, len(chunks))
	for _, chunk := range chunks {
		result := make(map[string]interface{}, len(chunk)+2)
		for key, value := range chunk {
			result[key] = value
		}
		if _, ok := result["chunk_id"]; !ok {
			if value, exists := chunk["id"]; exists {
				result["chunk_id"] = value
			} else if value, exists := chunk["_id"]; exists {
				result["chunk_id"] = value
			}
		}
		if value, ok := chunk["content"]; ok {
			if _, exists := result["content_ltks"]; !exists {
				result["content_ltks"] = value
			}
			if _, exists := result["content_with_weight"]; !exists {
				result["content_with_weight"] = value
			}
		}
...
// GetChunkRequest request for getting a single chunk
type GetChunkRequest struct {
	ChunkID string `json:"chunk_id"`
}

// GetChunkResponse response for getting a single chunk
type GetChunkResponse struct {
	Chunk map[string]interface{} `json:"chunk"`
}

// Get retrieves a single chunk by ID across the user's accessible tenants.
func (s *ChunkService) Get(req *GetChunkRequest, userID string) (*GetChunkResponse, error) {
	...
	for _, tenant := range tenants {
		...
		indexName := fmt.Sprintf("multirag_%s", tenant.TenantID)
...
// ListChunksRequest request for listing chunks of a document
...
// List retrieves the chunks belonging to a document.
func (s *ChunkService) List(req *ListChunksRequest, userID string) (*ListChunksResponse, error) {
	...
	indexName := fmt.Sprintf("multirag_%s", targetTenantID)
	page := getPageNum(req.Page, 1)
		...
		for k, v := range chunk {
			...
			case "img_id":
				if strVal, ok := v.(string); ok {
					formatted["image_id"] = strVal
				} else {
					formatted["image_id"] = ""
				}
			case "position_int":
				formatted["positions"] = v
			case "id":
				formatted["chunk_id"] = v
			case "content":
				formatted["content_with_weight"] = v
			case "docnm":
				formatted["docnm_kwd"] = v
			case "important_keywords":
				utility.SetFieldArray(formatted, "important_kwd", v)
			case "questions":
				utility.SetFieldArray(formatted, "question_kwd", v)
			case "entities_kwd", "entity_kwd", "entity_type_kwd", "from_entity_kwd",
				"name_kwd", "raptor_kwd", "removed_kwd",
				"source_id", "tag_kwd", "to_entity_kwd", "toc_kwd", "doc_type_kwd":
				if utility.IsEmpty(v) {
					formatted[k] = []interface{}{}
				} else {
					formatted[k] = v
				}
			default:
...
// UpdateChunk updates a chunk's fields
func (s *ChunkService) UpdateChunk(req *UpdateChunkRequest, userID string) error {
	...
	// Build update dict like Python does (doc.py update_chunk)
