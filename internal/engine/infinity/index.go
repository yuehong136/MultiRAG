...
func (o *orderedFields) UnmarshalJSON(data []byte) error {
	// Parse JSON manually to preserve key order using a streaming decoder
...
// indexName is the table name prefix (e.g., "multirag_<tenant_id>")
...
func (e *infinityEngine) CreateIndex(ctx context.Context, indexName, datasetID string, vectorSize int, parserID string) error {
	...
	fpMapping := filepath.Join(utility.GetProjectBaseDirectory(), "configs", e.mappingFileName)
...
func (e *infinityEngine) CreateDocMetaIndex(ctx context.Context, indexName string) error {
	...
	fpMapping := filepath.Join(utility.GetProjectBaseDirectory(), "configs", e.docMetaMappingFileName)
...
// TransformChunkFields transforms chunk field names/values for insert/update.
...
// buildFilterFromCondition builds an Infinity SQL filter from a condition map for UpdateDataset.
// Unlike search.go's equivalentConditionToStr (which only processes "_"-prefixed keys for the
// search path), this processes the actual condition keys (id, kb_id, tag_kwd, ...).
...
// Transform chunks before insert (see TransformChunkFields)
...
// UpdateMetadata merges metaFields into a document's row in the tenant's metadata table
// Table name format: multirag_doc_meta_{tenant_id}
// Existing meta_fields are read and merged with the new values (new keys win).
func (e *infinityEngine) UpdateMetadata(ctx context.Context, docID string, kbID string, metaFields map[string]interface{}, tenantID string) error {
	...
	// Query existing metadata to merge with. Our Infinity SDK returns a
	// *infinity.QueryResult (column-oriented Data), so read meta_fields[0] from it
	// rather than upstream's row-oriented []map assertion.
	...
	if err != nil {
		...
		// If query fails, just write the new metadata
	} else if qr, ok := queryResult.(*infinity.QueryResult); ok {
		if metaArr, ok := qr.Data["meta_fields"]; ok && len(metaArr) > 0 {
			...
			case []byte:
				if err := json.Unmarshal(v, &existingMetaFields); err != nil {
					existingMetaFields = make(map[string]interface{})
				}
			case map[string]interface{}:
			...
			// Merge new meta_fields onto existing (new keys win)
