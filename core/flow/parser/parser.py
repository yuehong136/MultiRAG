...
class Parser(ProcessBase):
        ...
        if parse_method.lower() == "deepdoc":
            ...
            def resolve_paddleocr_llm_name():
                ...
                with db_connection() as db:
                    ...
                    candidates = TenantLLMService.query(db, tenant_id=tenant_id, llm_factory="PaddleOCR", mdl_type=LLMType.OCR.value)
                    if candidates:
                        return candidates[0].llm_name
                    return env_name
    ...
    def _audio(self, name, blob, **kwargs):
        ...
        with tempfile.NamedTemporaryFile(suffix=ext) as tmpf:
            ...
            with db_connection() as db:
                seq2txt_config = get_model_config_by_type_and_name(db, self._canvas.get_tenant_id(), LLMType.SPEECH2TEXT.value, vlm["llm_id"])
    ...
    def _video(self, name, blob, **kwargs):
        ...
        self.set_output("output_format", conf["output_format"])
        with db_connection() as db:
            cv_config = get_model_config_by_type_and_name(db, self._canvas.get_tenant_id(), LLMType.IMAGE2TEXT.value, vlm["llm_id"])
