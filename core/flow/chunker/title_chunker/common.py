...
from core.nlp import not_bullet, not_title
    ...
    def resolve_outline_levels(self, line_records):
        ...
        for record in line_records:
            ...
            for outline in outlines:
                outline_text, level = outline[0], outline[1]
            ...
            if records[0]["doc_type_kwd"] == "text":
                ...
                for record in records:
                    if record.get("img_id"):
                        chunk["img_id"] = record["img_id"]
                        break
                for record in records:
                    if record.get("image") is not None:
                        chunk["image"] = record["image"]
                        break
            else:
                chunk = {
                    "text": records[0]["text"],
                ...
                if records[0].get("image") is not None:
