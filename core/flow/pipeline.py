    ...
    def __init__(self, dsl: str, tenant_id=None, doc_id=None, task_id=None, flow_id=None):
        ...
        if doc_id:
            with db_connection() as db:
                self._kb_id = DocumentService.get_knowledgebase_id(db, doc_id)
        ...
        with db_connection() as db:
            if self._doc_id:
                DocumentService.update_by_id(
                    db, self._doc_id, {"progress": random.randint(0, 5) / 100.0, "progress_msg": "Start the pipeline...", "process_begin_at": datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")}
                )
            ...
            if self._doc_id:
                DocumentService.update_by_id(db, self._doc_id, {"progress": 1 if not self.error else -1, "progress_msg": "Pipeline finished...\n" + self.error, "process_duration": time.perf_counter() - st})
