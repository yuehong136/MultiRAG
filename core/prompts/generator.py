...
def memory_prompt(message_list, max_tokens):
    """
    Format message list into prompt content with token budget.

    Args:
        message_list: List of message dicts with "content" field
        max_tokens: Maximum token budget

    Returns:
        List of message content strings within token limit
    """
    used_token_count = 0
    content_list = []
...
async def cross_languages(tenant_id, llm_id, query, languages=None):
    ...
    with db_connection() as db:
        if llm_id and TenantLLMService.llm_id2llm_type(llm_id) == "image2text":
            ...
            if not llm_id:
                mdl_config = get_tenant_default_model_by_type(db, tenant_id, LLMType.CHAT.value)
            else:
                mdl_config = get_model_config_by_type_and_name(db, tenant_id, LLMType.CHAT.value, llm_id)
        chat_mdl = LLMBundle(db, tenant_id, mdl_config)
...
async def reflect_async(chat_mdl, history: list[dict], tool_call_res: list[tuple], user_defined_prompts: dict = {}):
    ...
    result = await chat_mdl.async_chat(msg[0]["content"], msg[1:])
