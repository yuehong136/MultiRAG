...
def analyze_task(chat_mdl, task_name, tools_description: list[dict]):
    ...
    result = chat_mdl.chat(ANALYZE_TASK_SYSTEM, [{"role": "user",
                                                  "content": template.render(task=task_name, context=context,
...
def reflect(chat_mdl, history: list[dict], tool_call_res: list[Tuple]):
    ...
    result = chat_mdl.chat(msg[0]["content"], msg[1:])
...
def rank_memories(chat_mdl, goal: str, sub_goal: str, tool_call_summaries: list[str]):
    ...
    user_prompt = " → rank: "
    _, msg = message_fit_in(form_message(system_prompt, user_prompt), chat_mdl.max_length)
    result = chat_mdl.chat(msg[0]["content"], msg[1:], stop="<|stop|>")
