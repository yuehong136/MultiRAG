...
from core.utils.file_utils import extract_embed_file, extract_html, extract_links_from_docx, extract_links_from_pdf
    ...
    **kwargs,
    ...
    if tenant_id:
        if not mineru_llm_name:
            try:
                ...
                with db_connection() as db:
                    ...
                    candidates = TenantLLMService.query(db, tenant_id=tenant_id, llm_factory="MinerU", mdl_type=LLMType.OCR)
                    if candidates:
        ...
        if mineru_llm_name:
            try:
                with db_connection() as db:
                    model_config = get_model_config_by_type_and_name(db, tenant_id, LLMType.OCR.value, mineru_llm_name)
    ...
    **kwargs,
    ...
    if tenant_id:
        if not paddleocr_llm_name:
            try:
                ...
                with db_connection() as db:
                    ...
                    candidates = TenantLLMService.query(db, tenant_id=tenant_id, llm_factory="PaddleOCR", mdl_type=LLMType.OCR)
                    if candidates:
        ...
        if paddleocr_llm_name:
            try:
                with db_connection() as db:
                    model_config = get_model_config_by_type_and_name(db, tenant_id, LLMType.OCR.value, paddleocr_llm_name)
                    ocr_model = LLMBundle(db, tenant_id, model_config, lang=lang)
                    pdf_parser = ocr_model.mdl
                    sections, tables = pdf_parser.parse_pdf(
                        filepath=filename,
...
def by_plaintext(filename, binary=None, from_page=0, to_page=100000, callback=None, **kwargs):
    ...
    if (not layout_recognizer) or (layout_recognizer == "Plain Text"):
        ...
        with db_connection() as db:
            model_config = get_model_config_by_type_and_name(db, tenant_id, LLMType.IMAGE2TEXT.value, layout_recognizer)
    ...
    if re.search(r"\.docx$", filename, re.IGNORECASE):
        ...
        try:
            with db_connection() as db:
                ...
                vision_model = LLMBundle(db, kwargs["tenant_id"], model_config)
