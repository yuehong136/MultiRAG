...
def chunk(filename, binary=None, lang="Chinese", callback=None, **kwargs):
    ...
    if re.search(r"\.xlsx?$", filename, re.IGNORECASE):
        ...
        while i < len(lines):
            ...
            i += 1
            if len(res) % 999 == 0:
                callback(len(res) * 0.6 / len(lines), (f"Extract TAG: {len(res)}" + (f"{len(fails)} failure, line: %s..." % (",".join(fails[:3])) if fails else "")))

        callback(0.6, (f"Extract TAG: {len(res)}" + (f"{len(fails)} failure, line: %s..." % (",".join(fails[:3])) if fails else "")))
        ...
        for i, row in enumerate(reader):
            ...
            if len(row) != 2:
                ...
                content = ""
            if len(res) % 999 == 0:
                callback(len(res) * 0.6 / len(lines), (f"Extract Tags: {len(res)}" + (f"{len(fails)} failure, line: %s..." % (",".join(fails[:3])) if fails else "")))
...
def label_question(db, question, kbs):
    ...
    if tag_kb_ids:
        ...
        if not tag_kbs:
            return tags
        tags = settings.retriever.tag_query(question, list({kb.tenant_id for kb in tag_kbs}), tag_kb_ids, all_tags, kb.parser_config.get("topn_tags", 3))
    return tags
