...
class EncryptedStorageWrapper:
    ...
    def __init__(self, storage_impl, algorithm="aes-256-cbc", key=None, iv=None):
        ...
        Args:
            ...
            key: Encryption key, uses MultiRAG_CRYPTO_KEY environment variable if None
    ...
    def put(self, bucket, fnm, binary, tenant_id=None):
        ...
        try:
            ...
            logging.exception(f"Failed to encrypt and store data: {bucket}/{fnm}, error: {e!s}")
            raise
...
def create_encrypted_storage(storage_impl, algorithm=None, key=None, encryption_enabled=True):
    ...
    Args:
        storage_impl: Original storage implementation instance
        algorithm: Encryption algorithm, uses environment variable MultiRAG_CRYPTO_ALGORITHM or default if None
        key: Encryption key, uses environment variable MultiRAG_CRYPTO_KEY if None
