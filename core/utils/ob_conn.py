...
class OBConnection(OBConnectionBase):
    def __init__(self):
        super().__init__(logger_name="multirag.ob_conn")
    ...
    def search(
        ...
        **kwargs,
        ...
        if "*" in output_fields:
            if index_names[0].startswith("multirag_doc_meta_"):
    ...
    def get(self, chunk_id: str, index_name: str, knowledgebase_ids: list[str]) -> dict | None:
        try:
            ...
            return {"id": chunk_id, "error": f"Failed to parse chunk data due to invalid JSON: {e!s}"}
        except Exception as e:
    ...
    def insert(self, documents: list[dict], index_name: str, knowledgebase_id: str = None) -> list[str]:
        ...
        if index_name.startswith("multirag_doc_meta_"):
    ...
    def update(self, condition: dict, new_value: dict, index_name: str, knowledgebase_id: str) -> bool:
        ...
        if not index_name.startswith("multirag_doc_meta_"):
    ...
    def adjust_chunk_pagerank_fea(
        ...
        sql = (
            ...
            f"GREATEST({int(min_w)}, LEAST({int(max_w)}, COALESCE({PAGERANK_FLD}, 0) + ({int(delta)}))) "
