...
def main():
    ...
    for i, step in enumerate(st.session_state.steps):
        ...
        if selected_step == "数据处理":
            st.session_state[f"workflow_params_{i}"] = {"method": cols[0].selectbox("选择处理方式", ["数值列求和", "数值列求平均"], key=f"processing_method_{i}")}
        ...
        for i, step in enumerate(st.session_state.steps):
            if i not in steps_to_remove:
                ...
                if f"workflow_params_{i}" in st.session_state:
                    new_workflow_params[f"workflow_params_{len(new_steps) - 1}"] = st.session_state[f"workflow_params_{i}"]
