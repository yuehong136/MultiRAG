...
if __name__ == "__main__":
    # 定义工作流
    workflow1 = Workflow()
    workflow1.define_start_outputs([OutputDefinition("string", "BOT_USER_INPUT", False, "用户本轮对话输入内容"), OutputDefinition("string", "INPUT", True, "测试input")])

    # 另一个工作流实例
    workflow2 = Workflow()
    workflow2.define_start_outputs([OutputDefinition("string", "ANOTHER_INPUT", True, "另一个输入")])

    # 执行工作流
    try:
        result1 = workflow1.execute({"BOT_USER_INPUT": "你好", "INPUT": "测试数据"})
        print("工作流1执行成功:", result1)

        result2 = workflow2.execute({"ANOTHER_INPUT": "其他数据"})
