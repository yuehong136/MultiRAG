            ...
            if file_name.endswith(".pdf"):
                ...
                with Image.open(file_path) as image:
                    ...
                    model_choice = st.selectbox(
                        "选择图像描述模型:", ["zhipu_4v", "qwen_cv", "gpt_v4", "ollama_cv", "xinference_cv", "local_cv"], key=index_keys, help="选择VLLM模型,目前只做了zhipu_4v的适配"
                    )
                    # # API key input
                    model_key = st.session_state.api_token if st.session_state.api_token and model_choice == "zhipu_4v" else st.text_input("输入 API key:", type="password")
                    model_name = ""
                    # # Optional parameters
...
with st.sidebar:
    ...
    def get_weather(city_name: str) -> str:
        ...
        try:
            ...
            weather_text = f"温度: {ret['temp_C']}°C\n体感温度: {ret['FeelsLikeC']}°C\n湿度: {ret['humidity']}%\n天气状况: {ret['weatherDesc'][0]['value']}\n观测时间: {ret['observation_time']}\n"
    ...
    with tab1:
        ...
        if dialogue_mode == "知识库问答":
            ...
            with st.expander("知识库参数", expanded=True):
                st.selectbox("Database", kb_list, on_change=on_mode_change, key="selected_kb", placeholder="选择一个知识库")
        ...
        display_files = [os.path.splitext(f)[0].split("-", 1)[1] for f in history_files]  # Remove timestamp and .json extension
        ...
        def handle_session_saving(selected_file_full_path=None):
            ...
            if st.button("确认保存", key="confirm_save"):
                ...
                if "from_history" in st.session_state and st.session_state.from_history:
                    # 从历史会话加载并保存
                    save_chat_history(st.session_state.messages, custom_name, file_path=selected_file_full_path, rename=bool(custom_name))
                else:
                    # 新建会话并保存
        ...
        if export_btn.button("导出对话", use_container_width=True):
            ...
            st.download_button(label="下载对话记录", data=export_callback(), file_name=f"{now:%Y-%m-%d %H.%M}_对话记录.md", mime="text/markdown", use_container_width=True)
        ...
        if model_name != st.session_state.previous_model_name:
            ...
            st.toast(f"欢迎使用 [`Datav-RAG`](https://dcs.dataonv.com/#/home) ! \n\n当前运行的模型`{model_name}`.")
    ...
    with tab3:
        ...
        toggle_feature(toggle_name="智能话题推荐", feature_name="is_recmd", on_message="智能话题推荐已开启！", off_message="智能话题推荐已关闭！", help="开启后，每回合LLM回复后会自动推荐3个话题")

        toggle_feature(toggle_name="BI模式", feature_name="is_mode_bi", on_message="BI模式已开启！", off_message="BI模式已关闭！", help="开启后，每回合LLM回复后默认返回BI问题按钮")
...
if prompt := st.chat_input("请输入您的问题："):
    ...
    if tool_call_match:
        ...
        st.session_state.messages.append(
            ...
            {"role": "tool", "content": "正在调用工具...", "tool_calls": [{"name": tool_name, "arguments": processed_prompt}]}
        )
    ...
    response_content = get_ai_response(api_token, model, messages, temperature, max_tokens, routing_instructions, tools)
    ...
    if recmd_option and st.session_state.option_recmd != recmd_option:  # 确保在选择单选框后更新内容
        if recmd_option == "数据对话":
            ...
            response_content = get_ai_response(
                st.session_state.api_token, st.session_state.model, st.session_state.messages, st.session_state.temperature, st.session_state.max_tokens, st.session_state.sys_prompt, tools
            )
            st.session_state.messages.append({"role": "assistant", "content": response_content})
