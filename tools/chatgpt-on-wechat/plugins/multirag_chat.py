...
class MultiRAGChat(Plugin):
    ...
    def get_ragflow_reply(self, user_input, session_id):
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if data.get("code") == 0:
                        ...
                        logging.error(f"[MultiRAGChat] Failed to create conversation: {data.get('message')}")
                        return f"Sorry, unable to create a conversation: {data.get('message')}"
                else:
                    logging.error(f"[MultiRAGChat] HTTP error when creating conversation: {response.status_code}")
        ...
        try:
            ...
            logging.debug(f"[MultiRAGChat] Completion response: {response.text}")
            if response.status_code == 200:
                data = response.json()
                if data.get("code") == 0:
                    ...
                    logging.error(f"[MultiRAGChat] Failed to get answer: {data.get('message')}")
                    return f"Sorry, unable to get a reply: {data.get('message')}"
            else:
                logging.error(f"[MultiRAGChat] HTTP error when getting answer: {response.status_code}")
