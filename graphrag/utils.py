...
def get_entity(tenant_id, kb_id, ent_name):
    ...
    res = []
    db = SessionLocal()
    milvus_res = settings.retrievaler.search(conds, search.index_name(tenant_id, [KnowledgebaseService.get_by_id(db, kb_id).name]), [kb_id])
    for id in milvus_res.ids:
        try:
            if isinstance(ent_name, str):
                set_entity_cache(tenant_id, kb_id, ent_name, milvus_res.field[id]["content_with_weight"])
                return json.loads(milvus_res.field[id]["content_with_weight"])
            res.append(json.loads(milvus_res.field[id]["content_with_weight"]))
        except Exception:
            continue
...
def set_entity(tenant_id, kb_id, embd_mdl, ent_name, meta):
    ...
    if res.ids:
        settings.docStoreConn.update({"entity_kwd": ent_name}, chunk, search.index_name(tenant_id, [KnowledgebaseService.get_by_id(db, kb_id).name]), kb_id)
    else:
...
def get_relation(tenant_id, kb_id, from_ent_name, to_ent_name, size=1):
    ...
    res = []
    db = SessionLocal()
    milvus_res = settings.retrievaler.search(conds, search.index_name(tenant_id, [KnowledgebaseService.get_by_id(db, kb_id).name]), [kb_id] if isinstance(kb_id, str) else kb_id)
    for id in milvus_res.ids:
        try:
            if size == 1:
                return json.loads(milvus_res.field[id]["content_with_weight"])
...
async def does_graph_contains(tenant_id, kb_id, doc_id):
    ...
    db = SessionLocal()
    res = await trio.to_thread.run_sync(lambda: settings.docStoreConn.search(fields, [], condition, [], OrderByExpr(), 0, 1, search.index_name(tenant_id, [KnowledgebaseService.get_by_id(db, kb_id).name]), [kb_id]))
...
async def get_graph_doc_ids(tenant_id, kb_id) -> list[str]:
    ...
    db = SessionLocal()
    res = await trio.to_thread.run_sync(lambda: settings.retrievaler.search(conds, search.index_name(tenant_id, [KnowledgebaseService.get_by_id(db, kb_id).name]), [kb_id]))
    doc_ids = []
    if res.total == 0:
        return doc_ids
    for id in res.ids:
...
async def set_graph(tenant_id, kb_id, graph, docids):
    ...
    res = await trio.to_thread.run_sync(lambda: settings.retrievaler.search({"knowledge_graph_kwd": "graph", "size": 1, "fields": []}, search.index_name(tenant_id, [KnowledgebaseService.get_by_id(db, kb_id).name]), [kb_id]))
    if res.ids:
        await trio.to_thread.run_sync(lambda: settings.docStoreConn.update({"knowledge_graph_kwd": "graph"}, chunk,
                                     search.index_name(tenant_id, [KnowledgebaseService.get_by_id(db, kb_id).name]), kb_id))
    else:
        await trio.to_thread.run_sync(lambda: settings.docStoreConn.insert([{"id": chunk_id(chunk), **chunk}], search.index_name(tenant_id, [KnowledgebaseService.get_by_id(db, kb_id).name]), kb_id))
...
async def update_nodes_pagerank_nhop_neighbour(tenant_id, kb_id, graph, n_hop):
    ...
    res = await trio.to_thread.run_sync(
        lambda: settings.retrievaler.search({"knowledge_graph_kwd": "ty2ents", "size": 1, "fields": []},
                                            search.index_name(tenant_id, [KnowledgebaseService.get_by_id(db, kb_id).name]), [kb_id]))
    if res.ids:
        ...
        await trio.to_thread.run_sync(
            lambda: settings.docStoreConn.insert([{"id": chunk_id(chunk), **chunk}], search.index_name(tenant_id, [KnowledgebaseService.get_by_id(db, kb_id).name]), kb_id))


async def get_entity_type2sampels(idxnms, kb_ids: list):
    milvus_res = await trio.to_thread.run_sync(lambda: settings.retrievaler.search({"knowledge_graph_kwd": "ty2ents", "kb_id": kb_ids,
                                       "size": 10000,
                                       "fields": ["content_with_weight"]},
                                      idxnms, kb_ids))
...
async def rebuild_graph(tenant_id, kb_id):
    ...
    for i in range(0, 39*bs, bs):
        milvus_res = await trio.to_thread.run_sync(lambda: settings.docStoreConn.search(flds, [],
                                 {"kb_id": kb_id, "knowledge_graph_kwd": ["entity", "relation"]},
                                 [],
                                 OrderByExpr(),
                                 i, bs, search.index_name(tenant_id, [KnowledgebaseService.get_by_id(db, kb_id).name]), [kb_id]
                                 ))
        tot = settings.docStoreConn.getTotal(milvus_res)
        if tot == 0:
            return None, None

        milvus_res = settings.docStoreConn.getFields(milvus_res, flds)
        for id, d in milvus_res.items():
