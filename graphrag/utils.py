...
def set_graph(tenant_id, kb_id, graph, docids):
    ...
    res = settings.retrievaler.search({"knowledge_graph_kwd": "graph", "size": 1, "fields": []}, search.index_name(tenant_id, [KnowledgebaseService.get_by_id(db, kb_id).name]), [kb_id])
    if res.ids:
...
def get_entity_type2sampels(idxnms, kb_ids: list):
    milvus_res = settings.retrievaler.search({"knowledge_graph_kwd": "ty2ents", "kb_id": kb_ids,
                                       "size": 10000,
                                       "fields": ["content_with_weight"]},
                                      idxnms, kb_ids)
