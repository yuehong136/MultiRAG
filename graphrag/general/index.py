...
async def generate_subgraph(
    ...
    await trio.to_thread.run_sync(
        lambda: settings.docStoreConn.delete(
            {"knowledge_graph_kwd": "subgraph", "source_id": doc_id}, search.index_name(tenant_id, [KnowledgebaseService.get_by_id(db, kb_id).name]), kb_id
        )
    )
    await trio.to_thread.run_sync(
        lambda: settings.docStoreConn.insert(
            [{"id": cid, **chunk}], search.index_name(tenant_id, [KnowledgebaseService.get_by_id(db, kb_id).name]), kb_id
        )
    )
    now = trio.current_time()
...
async def extract_community(
    ...
    await trio.to_thread.run_sync(
        lambda: settings.docStoreConn.delete(
            ...
            search.index_name(tenant_id, [KnowledgebaseService.get_by_id(db, kb_id).name]),
            kb_id,
        )
    )
    es_bulk_size = 4
    for b in range(0, len(chunks), es_bulk_size):
        doc_store_result = await trio.to_thread.run_sync(
            lambda: settings.docStoreConn.insert(chunks[b:b + es_bulk_size], search.index_name(tenant_id, [KnowledgebaseService.get_by_id(db, kb_id).name]), kb_id))
        if doc_store_result:
