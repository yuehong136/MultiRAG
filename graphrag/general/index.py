    ...
    async def __call__(self):
        with RedisDistributedLock(self.kb_id, 60 * 60):
            ...
            await trio.to_thread.run_sync(
                lambda: update_nodes_pagerank_nhop_neighbour(self.tenant_id, self.kb_id, self.graph, 2))
        ...
        }, search.index_name(self.tenant_id, [KnowledgebaseService.get_by_id(db, self.kb_id).name]), self.kb_id))
        await trio.to_thread.run_sync(lambda: settings.docStoreConn.delete({
            "knowledge_graph_kwd": "relation",
            "kb_id": self.kb_id,
    ...
    async def __call__(self):
        with RedisDistributedLock(self.kb_id, 60 * 60):
            ...
            cr = CommunityReportsExtractor(self.llm_bdl,
                                           ...
                                           set_entity=partial(set_entity, self.tenant_id, self.kb_id, self.embed_bdl),
                                           get_relation=partial(get_relation, self.tenant_id, self.kb_id),
                                           set_relation=partial(set_relation, self.tenant_id, self.kb_id,
                                                                self.embed_bdl))
        ...
        if self.callback:
            self.callback(
                msg="Graph community extraction is done. Indexing {} reports.".format(len(cr.structured_output)))
        ...
        for stru, rep in zip(self.community_structure, self.community_reports):
            ...
            await trio.to_thread.run_sync(lambda: settings.docStoreConn.insert([{"id": chunk_id(chunk), **chunk}], search.index_name(self.tenant_id, [KnowledgebaseService.get_by_id(db, self.kb_id).name])))
