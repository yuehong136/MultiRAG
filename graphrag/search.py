...
class KGSearch(Dealer):
    ...
    def _ent_info_from_(self, milvus_res, sim_thr=0.3):
        res = {}
        milvus_res = self.dataStore.getFields(milvus_res, ["content_with_weight", "_score", "entity_kwd", "rank_flt",
                                                   "n_hop_with_weight"])
    ...
    def retrieval(self, question: str,
        ...
        ents = []
        try:
            ty_kwds, ents = self.query_rewrite(llm, qst, [index_name(tid, kb_names) for tid in tenant_ids], kb_ids)
