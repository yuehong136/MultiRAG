...
from api.utils.api_utils import get_data_error_result, get_error_data_result, get_json_result, get_parser_config, server_error_response
from api.utils.tenant_utils import ensure_tenant_model_id_for_params
from common import settings
from common.constants import VALID_TASK_STATUS, FileSource, LLMType, PipelineTaskType, RetCode, StatusEnum
        ...
        if kb.pagerank != req_data.get("pagerank", 0):
            ...
            if req_data.get("pagerank", 0) > 0:
                ...
                try:
                    ...
                    logging.error(f"移除知识库 {kb.id} 的 PageRank 失败: {e!s}")

        # 处理 connectors 关联
        if connectors:
            errors = Connector2KbService.link_connectors(db, kb.id, list(connectors), user.id)
            if errors:
...
def run_graphrag(request: RunGraphRagRequest, db: Session = Depends(get_db), user=Depends(manager)):
    ...
    task_id = queue_raptor_o_graphrag_tasks(db, sample_doc=sample_document, ty="graphrag", priority=0, fake_doc_id=GRAPH_RAPTOR_FAKE_DOC_ID, doc_ids=list(document_ids))
...
def run_raptor(request: RunRaptorRequest, db: Session = Depends(get_db), user=Depends(manager)):
    ...
    task_id = queue_raptor_o_graphrag_tasks(db, sample_doc=sample_document, ty="raptor", priority=0, fake_doc_id=GRAPH_RAPTOR_FAKE_DOC_ID, doc_ids=list(document_ids))
...
def run_mindmap(request: RunMindmapRequest, db: Session = Depends(get_db), user=Depends(manager)):
    ...
    task_id = queue_raptor_o_graphrag_tasks(db, sample_doc=sample_document, ty="mindmap", priority=0, fake_doc_id=GRAPH_RAPTOR_FAKE_DOC_ID, doc_ids=list(document_ids))
