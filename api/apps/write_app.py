...
def generate_outline(request: OutlineRequest, db: Session = Depends(get_db), user=Depends(manager)):
    """生成文章大纲"""
    try:
        # 生成大纲
        ...
        return {"outline_md": outline_result["outline_md"], "outline_structure": outline_result["outline_structure"], "article_id": outline_result["article_id"]}
...
def get_projects(page: int = 1, limit: int = 10, orderby: str = "create_time", desc: bool = True, keywords: str = None, db: Session = Depends(get_db), user=Depends(manager)):
    """获取用户的写作项目列表"""
    try:
        projects, total = ProjectService.get_list(db=db, user_id=user.id, page_number=page, items_per_page=limit, orderby=orderby, sort_desc=desc, keywords=keywords)

        return get_json_result(data={"items": projects, "total": total, "page": page, "limit": limit})
    except Exception as e:
...
def get_project_detail(project_id: str, db: Session = Depends(get_db), user=Depends(manager)):
    """获取项目详情"""
    try:
        project_detail = ProjectService.get_project_details(db=db, project_id=project_id, user_id=user.id)
...
def update_project(project_id: str, project_data: dict, db: Session = Depends(get_db), user=Depends(manager)):
    """更新项目信息"""
    try:
        updated_project = ProjectService.update_project(db=db, project_id=project_id, project_data=project_data, user_id=user.id)
...
def update_project_outline(project_id: str, outline_data: dict, db: Session = Depends(get_db), user=Depends(manager)):
    """
    更新项目的完整大纲结构
    ...
    try:
        # 确保有项目访问权限
        project = db.query(WritingProject).filter(WritingProject.id == project_id, WritingProject.status == StatusEnum.VALID.value, WritingProject.user_id == user.id).first()

        if not project:
            raise HTTPException(status_code=404, detail=f"项目不存在或无权访问: {project_id}")

        # 执行大纲更新
        updated_outline = ChapterService.update_project_outline(db=db, project_id=project_id, outline_data=outline_data)

        return get_json_result(data={"message": "大纲已成功更新", "outline": updated_outline})
    except Exception as e:
...
def delete_project(project_id: str, db: Session = Depends(get_db), user=Depends(manager)):
    """删除项目"""
    try:
        success = ProjectService.delete_project(db=db, project_id=project_id, user_id=user.id)
...
def add_chapter(project_id: str, chapter_data: ChapterRequest, db: Session = Depends(get_db), user=Depends(manager)):
    """添加章节"""
    try:
        # 确保有项目访问权限
        ...
        new_chapter = ChapterService.create_chapter(db=db, chapter_data=chapter_dict)
...
def get_project_chapters(project_id: str, db: Session = Depends(get_db), user=Depends(manager)):
    """获取项目的章节列表"""
    try:
        # 确保有项目访问权限
        project = ProjectService.get_project_details(db, project_id, user.id)
...
def delete_chapter_references(chapter_id: str, db: Session = Depends(get_db), user=Depends(manager)):
    """删除章节的所有参考资料"""
    try:
        # 验证章节是否存在
        ...
        if not chapter:
            return get_json_result(data=False, retmsg=f"章节不存在: {chapter_id}", retcode=404)

        # 验证项目所有权
        project = db.query(WritingProject).filter(WritingProject.id == chapter.project_id, WritingProject.status == StatusEnum.VALID.value).first()

        if not project or project.user_id != user.id:
            return get_json_result(data=False, retmsg="无权操作此章节的参考资料", retcode=403)

        # 删除章节的所有参考资料
        count = ReferenceService.delete_chapter_references(db, chapter_id)

        return get_json_result(data={"message": f"成功删除 {count} 个参考资料", "deleted_count": count})
    except Exception as e:
        logging.error(f"删除章节参考资料失败: {e!s}", exc_info=True)
        return get_json_result(data=False, retmsg=f"删除章节参考资料失败: {e!s}", retcode=500)


@router.put("/api/projects/{project_id}/outline/reorder")
def reorder_outline_sections(project_id: str, sections_order: list[dict[str, Any]], db: Session = Depends(get_db), user=Depends(manager)):
    """重新排序大纲章节"""
    try:
        # 确保有项目访问权限
        project = ProjectService.get_project_details(db, project_id, user.id)
        ...
        return get_json_result(data={"message": "章节顺序已更新", "outline": updated_outline})
    except Exception as e:
...
async def parse_reference_material(
    ...
    try:
        if file:
            ...
            # 检查文件是否为空
            if not file_content:
                return get_json_result(data=False, retmsg="文件内容为空", retcode=400)
            ...
            if not content or content.startswith("解析文件失败") or content.startswith("无法解析"):
                return get_json_result(data=False, retmsg=f"文件解析失败: {content}", retcode=400)
            ...
            if not url or not url.startswith(("http://", "https://")):
                return get_json_result(data=False, retmsg="无效的URL格式", retcode=400)
            ...
            if not content or content.startswith("解析URL内容失败"):
                return get_json_result(data=False, retmsg=f"URL解析失败: {content}", retcode=400)
            ...
            return get_json_result(data=False, retmsg="请提供文件、URL或文本内容", retcode=400)
        ...
        # 创建参考资料
        reference_data = {"chapter_id": chapter_id, "title": title, "content": content, "source": source, "type": ref_type}

        new_reference = ReferenceService.add_reference(db, reference_data)

        # 返回参考资料信息（不包括完整内容，只返回摘要）
        return get_json_result(
            data={"id": new_reference.id, "title": new_reference.title, "summary": ReferenceService.get_summary(content), "source": new_reference.source, "type": new_reference.type}
        )
    except Exception as e:
        logging.error(f"解析参考资料失败: {e!s}", exc_info=True)
        return get_json_result(data=False, retmsg=f"解析参考资料失败: {e!s}", retcode=500)


@router.get("/api/chapters/{chapter_id}/references")
def get_chapter_references(chapter_id: str, db: Session = Depends(get_db), user=Depends(manager)):
    """获取章节的参考资料列表"""
    try:
...
def delete_reference(reference_id: str, db: Session = Depends(get_db), user=Depends(manager)):
    """删除单个参考资料"""
    try:
        # 获取参考资料信息
        reference = db.query(WritingReferenceMaterial).filter(WritingReferenceMaterial.id == reference_id, WritingReferenceMaterial.status == StatusEnum.VALID.value).first()

        if not reference:
            return get_json_result(data=False, retmsg=f"参考资料不存在: {reference_id}", retcode=404)

        # 获取章节信息
        chapter = db.query(WritingChapter).filter(WritingChapter.id == reference.chapter_id, WritingChapter.status == StatusEnum.VALID.value).first()

        if not chapter:
            return get_json_result(data=False, retmsg="参考资料关联的章节不存在", retcode=404)

        # 验证项目所有权
        project = db.query(WritingProject).filter(WritingProject.id == chapter.project_id, WritingProject.status == StatusEnum.VALID.value).first()

        if not project or project.user_id != user.id:
            return get_json_result(data=False, retmsg="无权删除此参考资料", retcode=403)

        # 删除参考资料
        success = ReferenceService.delete_reference(db, reference_id)

        if not success:
            return get_json_result(data=False, retmsg="删除参考资料失败", retcode=500)

        return get_json_result(data={"message": "参考资料已成功删除"})
    except Exception as e:
        logging.error(f"删除参考资料失败: {e!s}", exc_info=True)
        return get_json_result(data=False, retmsg=f"删除参考资料失败: {e!s}", retcode=500)
...
def write_section(
    ...
    try:
        if not chapter_id:
            return get_json_result(retcode=400, retmsg="必须提供有效的章节ID", data=None)

        # 获取章节信息
        chapter = db.query(WritingChapter).filter(WritingChapter.id == chapter_id, WritingChapter.status == StatusEnum.VALID.value).first()

        if not chapter:
            return get_json_result(retcode=404, retmsg=f"未找到章节 ID: {chapter_id}", data=None)

        # 验证项目所有权
        project = db.query(WritingProject).filter(WritingProject.id == chapter.project_id, WritingProject.status == StatusEnum.VALID.value).first()

        if not project or project.user_id != user.id:
            return get_json_result(retcode=403, retmsg="无权访问此章节", data=None)

        # 调用改进的写作服务方法（得到字典结果）
        result = WritingService.write_section_improved(db=db, chapter_id=chapter_id)

        # 将字典结果转换为JSONResponse
        return get_json_result(retcode=result["retcode"], retmsg=result["retmsg"], data=result["data"])
    except Exception as e:
        logging.error(f"章节写作出错: {e!s}", exc_info=True)
        return get_json_result(retcode=500, retmsg=f"章节写作失败: {e!s}", data={"type": "error"})
...
def write_section_stream(
    ...
    return StreamingResponse(
        WritingService.write_section_stream_improved(db=db, chapter_id=chapter_id),
...
def get_complete_article(article_id: str, db: Session = Depends(get_db), user=Depends(manager)):
    """获取完整文章内容"""
    try:
        # 确保有项目访问权限
        ...
        return get_json_result(data={"complete": complete, "article_id": article_id, "content": content, "word_count": word_count})
    except Exception as e:
...
def get_article_status(article_id: str, db: Session = Depends(get_db), user=Depends(manager)):
    """获取文章状态和进度"""
    try:
        # 获取项目大纲和完成情况
