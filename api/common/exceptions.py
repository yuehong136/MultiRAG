...
def setup_exception_handlers(app: FastAPI):
    ...
    async def admin_exception_handler(request: Request, exc: AdminException):
        ...
        return JSONResponse(status_code=status.HTTP_200_OK, content={"code": exc.code, "message": exc.message, "data": None})

    @app.exception_handler(RequestValidationError)
    async def validation_exception_handler(request: Request, exc: RequestValidationError):
        """处理请求验证异常"""
        logging.error(f"Validation error: {exc.errors()}")
        return JSONResponse(status_code=status.HTTP_200_OK, content={"code": 400, "message": f"Validation error: {exc.errors()}", "data": None})

    @app.exception_handler(StarletteHTTPException)
    async def http_exception_handler(request: Request, exc: StarletteHTTPException):
        """处理 HTTP 异常"""
        logging.error(f"HTTP error: {exc.detail}")
        return JSONResponse(status_code=status.HTTP_200_OK, content={"code": exc.status_code, "message": exc.detail, "data": None})

    @app.exception_handler(Exception)
    async def general_exception_handler(request: Request, exc: Exception):
        """处理通用异常"""
        logging.error(f"Unexpected error: {exc!s}", exc_info=True)
        return JSONResponse(status_code=status.HTTP_200_OK, content={"code": 500, "message": f"Internal server error: {exc!s}", "data": None})
