...
class WritingService(CommonService):
    ...
    def _build_writing_prompt(cls, db: Session, chapter_id: str,
        ...
        # 构建参考资料文本
        reference_text = ""
        if db_references or reference_materials:
            reference_text = "### 参考资料：\n\n"
            ...
            if reference_materials:
                start_idx = len(db_references) + 1
                for i, material in enumerate(reference_materials):
                    reference_text += f"{i + start_idx}. {material}\n\n"
...
1. 保持与已完成章节的连贯性和一致性
2. 遵循指定的语言风格
3. 章节长度控制在{chapter_word_count}字左右
4. 注重逻辑性和可读性
5. 直接以当前章节的内容开始，不要包含标题或编号
6. 利用章节摘要作为写作指导
7. 充分利用所提供的参考资料，但不要直接复制
...
"""
    ...
    async def write_section_stream(cls, db: Session, chapter_id: str, extra_references: List[str] = None):
        ...
        try:
            ...
            for chunk in response_generator:  # 使用普通的for循环而不是async for
                if chunk:
