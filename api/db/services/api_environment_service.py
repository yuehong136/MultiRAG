...
from .api_environment_models import (
    ...
    EnvironmentDuplicateRequest, BatchVariablesRequest, VariableResolveResponse,
...
class ApiEnvironmentService:
    ...
    def get_environments(
        ...
        try:
            ...
            raise Exception(f"获取环境列表失败: {str(e)}")
    ...
    def get_environment_detail(self, db: Session, tenant_id: str, environment_id: str) -> EnvironmentDetailResponse:
        ...
        try:
            ...
            raise Exception(f"获取环境详情失败: {str(e)}")
    ...
    def create_environment(self, db: Session, tenant_id: str, env_data: EnvironmentCreate) -> EnvironmentDetailResponse:
        ...
        try:
            ...
            db.rollback()
            logger.error(f"创建环境失败(完整性约束): {str(e)}")
            raise Exception("环境名称已存在")
        except SQLAlchemyError as e:
            db.rollback()
            logger.error(f"创建环境失败: {str(e)}")
            raise Exception(f"创建环境失败: {str(e)}")
    ...
    def update_environment(
        ...
        try:
            ...
            db.rollback()
            logger.error(f"更新环境失败(完整性约束): {str(e)}")
            raise Exception("环境名称已存在")
        except SQLAlchemyError as e:
            db.rollback()
    ...
    def duplicate_environment(
        ...
        try:
            ...
            db.rollback()
            logger.error(f"复制环境失败(完整性约束): {str(e)}")
            raise Exception("环境名称已存在")
        except SQLAlchemyError as e:
            db.rollback()
            logger.error(f"复制环境失败: {str(e)}")
            raise Exception(f"复制环境失败: {str(e)}")
    ...
    def set_default_environment(self, db: Session, tenant_id: str, environment_id: str) -> EnvironmentDetailResponse:
        ...
        try:
            ...
            raise Exception(f"设置默认环境失败: {str(e)}")
    ...
    def create_variable(
        ...
        try:
            ...
            db.rollback()
            logger.error(f"创建变量失败(完整性约束): {str(e)}")
            raise Exception("变量名已存在")
        except SQLAlchemyError as e:
            db.rollback()
            logger.error(f"创建变量失败: {str(e)}")
            raise Exception(f"创建变量失败: {str(e)}")
    ...
    def update_variable(
        ...
        try:
            ...
            db.rollback()
            logger.error(f"更新变量失败(完整性约束): {str(e)}")
            raise Exception("变量名已存在")
        except SQLAlchemyError as e:
            db.rollback()
            logger.error(f"更新变量失败: {str(e)}")
            raise Exception(f"更新变量失败: {str(e)}")
    ...
    def delete_variable(self, db: Session, tenant_id: str, environment_id: str, variable_id: str) -> bool:
        ...
        try:
            ...
            raise Exception(f"删除变量失败: {str(e)}")
    ...
    def batch_update_variables(
        ...
        try:
            ...
            raise Exception(f"批量更新变量失败: {str(e)}")
    ...
    def resolve_variables(
        ...
        try:
            ...
            logger.error(f"解析变量失败: {str(e)}")
            raise Exception(f"解析变量失败: {str(e)}")
    ...
    def get_global_environments(self, db: Session) -> list[GlobalEnvironmentResponse]:
        ...
        try:
            ...
            raise Exception(f"获取全局环境失败: {str(e)}")
