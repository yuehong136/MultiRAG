...
class AsyncLLMService:
    ...
    async def send_event(self, event_id: str, data: dict[str, Any], event_type: str = "message"):
        """发送事件的辅助方法"""
        try:
            await event_manager.publish(event_id=event_id, data=data, event_type=event_type)
        except Exception as e:
            logger.error(f"Failed to send event {event_type}: {e}")

    async def chat_stream_async(self, event_id: str, tenant_id: str, system: str = "", history: list = None, gen_conf: dict = None, llm_name: str | None = None) -> None:
        if history is None:
            history = []
        if gen_conf is None:
        ...
        try:
            await self.send_event(event_id, {"message": "开始生成回复", "status": "started"}, "chat_start")
            ...
            await self.send_event(event_id, {"message": "模型加载完成，开始流式生成", "model_name": llm_name or "default", "tenant_id": tenant_id}, "model_ready")

            await self._simple_streaming(event_id=event_id, llm_bundle=llm_bundle, system=system, history=history, gen_conf=gen_conf)

        except Exception as e:
            logger.exception(f"Async chat stream error - event_id: {event_id}")
            await self.send_event(event_id, {"message": f"聊天失败: {e!s}", "error": str(e), "status": "error"}, "chat_error")
            # SSE 生成器只在收到 stream_end 时关闭连接，错误后必须收尾，否则连接挂到30分钟超时
            await self.send_event(event_id, {"message": "Stream finished, closing connection."}, "stream_end")

    async def _simple_streaming(self, event_id: str, llm_bundle: LLMBundle, system: str, history: list, gen_conf: dict) -> None:
        """简化的流式处理，使用标准库队列"""
        ...
        try:
            ...
            while True:
                try:
                    ...
                    if chunk_type == "done":
                        ...
                        logger.info(f"分析流完成 - content_len={len(full_content)}, chunk_count={chunk_count} - event_id: {event_id}")
                        if chunk_count > 0 and len(full_content) == 0:
                            logger.warning(f"分析流只思考无正文 - chunk_count={chunk_count} 但最终正文为空（疑似推理模型以 </think> 收尾）- event_id: {event_id}")
                        await self.send_event(event_id, {"message": "生成完成", "tokens_used": 0, "chunk_count": chunk_count, "status": "completed"}, "chat_complete")
                        await self.send_event(event_id, {"content": full_content, "tokens_used": 0, "final": True}, "chat_result")
                        ...
                        await self.send_event(event_id, {"message": f"LLM生成错误: {chunk_data}", "status": "error"}, "chat_error")
                        await self.send_event(event_id, {"message": "Stream finished, closing connection."}, "stream_end")
                        return
                        ...
                        await self.send_event(event_id, {"content": delta_content, "chunk_index": chunk_count, "is_final": False}, "chat_content")

                except queue.Empty:
                    await asyncio.sleep(0.01)
                    if not worker_thread.is_alive() and chunk_queue.empty():
                        logger.warning(f"工作线程意外结束 - event_id: {event_id}")
                        await self.send_event(event_id, {"message": "LLM工作线程意外结束", "status": "error"}, "chat_error")
                        await self.send_event(event_id, {"message": "Stream finished, closing connection."}, "stream_end")
                        break
            ...
            error_occurred.set()
            await self.send_event(event_id, {"message": f"流式处理错误: {e!s}", "status": "error"}, "chat_error")
            await self.send_event(event_id, {"message": "Stream finished, closing connection."}, "stream_end")
        finally:
            error_occurred.set()
