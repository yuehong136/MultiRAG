...
class SQLComponentsParser:
    ...
    def parse_from_tables(self) -> list[dict[str, str]]:
        ...
        # 分割JOIN语句
        # 先处理整个FROM子句，识别所有的表
        parts = re.split(r"\s+(?:INNER\s+|LEFT\s+OUTER\s+|RIGHT\s+OUTER\s+|FULL\s+OUTER\s+|LEFT\s+|RIGHT\s+|FULL\s+|CROSS\s+)?JOIN\s+", from_content, flags=re.IGNORECASE)
    ...
    def parse_where_conditions(self) -> dict:
        ...
        if not where_clause:
            return {"has_or": False, "raw_condition": "", "parsed_conditions": []}
        ...
        result = {"has_or": has_or, "raw_condition": where_content, "parsed_conditions": []}
    ...
    def parse_having_conditions(self) -> dict:
        ...
        result = {"has_or": has_or, "raw_condition": having_content, "parsed_conditions": []}
    ...
    def parse_order_by(self) -> list[dict[str, str]]:
        ...
        for part in parts:
            ...
            order_fields.append({"field": field, "direction": direction})
    ...
    def _extract_table_info(self, table_expr: str) -> dict[str, str] | None:
        ...
        if as_match:
            return {"table": as_match.group(1), "alias": as_match.group(2)}
        ...
        if space_match:
            ...
            if not re.match(r"^(ON|WHERE|GROUP|ORDER|HAVING|LIMIT)$", potential_alias, re.IGNORECASE):
                return {"table": space_match.group(1), "alias": potential_alias}
...
if __name__ == "__main__":
    ...
    # 测试用例5：只有offset的特殊情况
    sql_components5 = {"select": "SELECT * ", "from": "FROM users", "where": "", "groupBy": "", "having": "", "orderBy": "ORDER BY created_at DESC", "pagination": {"limit": "", "offset": "100"}}
