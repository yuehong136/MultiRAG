...
class SemanticFilterProcessor:
    ...
    def process_excluded_fields(
        dimensions: list[dict], all_metrics: list[dict], exclude_dim_and_metric: dict[str, list[str]], log_details: bool = True
        ...
        # 收集被排除字段的详细信息
        excluded_fields_detail = {"excluded_dimensions": [], "excluded_metrics": [], "statistics": {}}
        ...
        dimensions[:] = [dim for dim in dimensions if dim.get("dimensionId") not in excluded_dim_ids]

        all_metrics[:] = [metric for metric in all_metrics if metric.get("metricId") not in excluded_metric_ids]

        # 更新统计信息
        excluded_fields_detail["statistics"] = {
            "original": {"dimensions": original_dim_count, "metrics": original_metric_count, "total": original_dim_count + original_metric_count},
            "excluded": {"dimensions": excluded_dim_count, "metrics": excluded_metric_count, "total": excluded_dim_count + excluded_metric_count},
            "retained": {"dimensions": len(dimensions), "metrics": len(all_metrics), "total": len(dimensions) + len(all_metrics)},
            "exclusion_rate": {
                ...
                "total": ((excluded_dim_count + excluded_metric_count) / (original_dim_count + original_metric_count) * 100) if (original_dim_count + original_metric_count) > 0 else 0,
            },
        }
        ...
        if log_details:
            ...
            logger.info(
                f"  - 原始维度数量: {original_dim_count}, 排除: {excluded_dim_count}, 保留: {len(dimensions)} (排除率: {excluded_fields_detail['statistics']['exclusion_rate']['dimensions']:.1f}%)"
            )
            logger.info(
                f"  - 原始指标数量: {original_metric_count}, 排除: {excluded_metric_count}, 保留: {len(all_metrics)} (排除率: {excluded_fields_detail['statistics']['exclusion_rate']['metrics']:.1f}%)"
            )
            logger.info(
                f"  - 总计: 原始 {original_dim_count + original_metric_count} 个字段, "
    ...
    def format_exclusion_summary(excluded_fields_detail: dict[str, list[dict]]) -> str:
        ...
        summary_lines = ["=== 语义过滤摘要 ===", f"排除维度: {excluded.get('dimensions', 0)} 个", f"排除指标: {excluded.get('metrics', 0)} 个", f"总计排除: {excluded.get('total', 0)} 个字段"]
...
def apply_semantic_filter(
    ...
    return processor.process_excluded_fields(dimensions, all_metrics, exclude_dim_and_metric, log_details)
