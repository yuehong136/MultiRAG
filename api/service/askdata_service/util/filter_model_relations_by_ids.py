...
def filter_model_relations_by_ids(model_ids: list[str], model_relations: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """
    ...
    filtered_relations = [relation for relation in model_relations if (relation.get("sourceModelId") in valid_model_ids_set or relation.get("targetModelId") in valid_model_ids_set)]

    # 打印统计信息，便于调试
    original_count = len(model_relations)
    filtered_count = len(filtered_relations)
    logger.info(f"原始关系数量: {original_count}, 过滤后剩余关系数量: {filtered_count}. 移除了 {original_count - filtered_count} 条无效关系。")
