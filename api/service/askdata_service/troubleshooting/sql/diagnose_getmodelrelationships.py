...
TABLES = [
    "t_drm_model_datamodel",
    "t_drm_datafield",
    "t_drm_model_dataitem",
    "t_drm_dataobject",
    "t_drm_model_dataset_child",
    "t_drm_model_dataset",
]
...
def diagnose_model(cur, mid, label, args):
    ...
    params = {"mid": mid}
    res = {"label": label, "mid": mid, "old_ms": None, "new_ms": None, "old_to": False, "new_to": False, "df_seqscan": False, "equiv": None}
    ...
    print(f"· v_old 实测: {'⚠ 超时 >' + str(args.timeout) + 's（复现慢查询！）' if old_to else f'{old_ms:.1f} ms'}" + ("  ⚠t_drm_datafield 走 Seq Scan" if res["df_seqscan"] else ""))
    ...
    if old_to or (old_ms is not None and old_ms > 5000):
        ...
        try:
            ...
            print(f"· 等价校验: old−new={d['a']}, new−old={d['b']}, 行数 old={d['o']}/new={d['n']} " + ("✅ 通过(结果一致)" if res["equiv"] else "❌ 不一致，禁止上线！"))
        except psycopg2.errors.QueryCanceled:
...
def main():
    ...
    ap.add_argument("--model-id", type=int, default=LOG_MODEL_ID, help=f"“指定”模型 wid（默认现场日志那次 {LOG_MODEL_ID}）")
    ...
    conn = psycopg2.connect(host=args.host, port=args.port, dbname=args.dbname, user=args.user, password=args.password, connect_timeout=10)
    ...
    if worst_to:
        ...
        fast = all((r["new_ms"] is not None and r["old_ms"] is not None and r["new_ms"] <= r["old_ms"] * 0.5) for r in results if r["old_ms"] is not None)
