        ...
        for element in doc.element.body:
            if isinstance(element, CT_P):
                ...
                if paragraph.text.strip():  # 只处理非空段落
                    ...
                    elements.append(ParagraphElement(content=paragraph.text, style=paragraph.style.name if paragraph.style else "Normal", alignment=paragraph.alignment, runs=runs))
                ...
                elements.append(TableElement(content=content, style=table.style.name if table.style else "TableNormal", row_count=len(table.rows), column_count=len(table.columns), html=table_html))
