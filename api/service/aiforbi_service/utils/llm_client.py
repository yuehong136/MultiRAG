...
from common.settings import AIFORBI_API_KEY, AIFORBI_BASE_URL, AIFORBI_MODEL_ID


class BaseLLMClient(ABC):
    def __init__(self, model_id: str):
        self.model_id = model_id
        self.base_url = AIFORBI_BASE_URL
        self.api_key = AIFORBI_API_KEY
...
class SyncLLMClient(BaseLLMClient):
    ...
    def standard_request(self, system_content: str | None = None, user_content: str = None) -> str:
        completion = self.client.chat.completions.create(model=self.model_id, messages=self._create_messages(system_content, user_content))
        return completion.choices[0].message.content

    def streaming_request(self, system_content: str | None, user_content: str) -> Generator[str, None, None]:
        stream = self.client.chat.completions.create(model=self.model_id, messages=self._create_messages(system_content, user_content), stream=True)
        for chunk in stream:
            if chunk.choices:
...
class AsyncLLMClient(BaseLLMClient):
    ...
    async def standard_request(self, system_content: str | None = None, user_content: str = None) -> str:
        completion = await self.client.chat.completions.create(model=self.model_id, messages=self._create_messages(system_content, user_content))
        return completion.choices[0].message.content

    async def streaming_request(self, system_content: str | None, user_content: str):
        stream = await self.client.chat.completions.create(model=self.model_id, messages=self._create_messages(system_content, user_content), stream=True)
