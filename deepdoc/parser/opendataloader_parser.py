...
Position = tuple[list[int], float, float, float, float]
CropPosition = tuple[int, float, float, float, float]
Section = tuple[str, ...]
TablePayload = tuple[tuple[Image.Image | None, str | list[str]], list[CropPosition] | str]
ParseResult = tuple[list[Section], list[TablePayload]]
...
def _bbox_from_element(el: dict[str, Any]) -> _BBox | None:
...
def _iter_elements(node: Any) -> Iterable[dict[str, Any]]:
...
def _element_html(el: dict[str, Any]) -> str:
...
class OpenDataLoaderParser(RAGFlowPdfParser):
    ...
    @staticmethod
    def extract_positions(txt: str) -> list[Position]:
        poss: list[Position] = []
    ...
    ) -> Image.Image | tuple[Image.Image | None, list[CropPosition] | None] | None:
    ...
    def _cropout_region(self, bbox: _BBox, zoomin: int = 1) -> tuple[Image.Image | None, list[CropPosition] | str]:
        if not self.page_images:
            return None, ""
    ...
    def _transfer_from_json(self, root: Any, parse_method: str) -> ParseResult:
    ...
    @staticmethod
    def _sections_from_markdown(md: str, parse_method: str) -> list[Section]:
        txt = (md or "").strip()
        if not txt:
            return []
        if parse_method in {"manual", "pipeline"}:
