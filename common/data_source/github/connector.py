...
from common.data_source.config import GITHUB_CONNECTOR_BASE_URL, DocumentSource
...
from .utils import deserialize_repository, get_external_access_permission
...
def _convert_pr_to_document(pull_request: PullRequest, repo_external_access: ExternalAccess | None) -> Document:
    ...
    return Document(
        ...
        doc_updated_at=(pull_request.updated_at.replace(tzinfo=UTC) if pull_request.updated_at else None),
        extension=".md",
        ...
        metadata={
            ...
            for k, v in {
                ...
                "created_at": (pull_request.created_at.replace(tzinfo=UTC) if pull_request.created_at else None),
                "updated_at": (pull_request.updated_at.replace(tzinfo=UTC) if pull_request.updated_at else None),
                "closed_at": (pull_request.closed_at.replace(tzinfo=UTC) if pull_request.closed_at else None),
                "merged_at": (pull_request.merged_at.replace(tzinfo=UTC) if pull_request.merged_at else None),
    ...
    return Document(
        ...
        metadata={
            ...
            for k, v in {
                ...
                "created_at": (issue.created_at.replace(tzinfo=UTC) if issue.created_at else None),
                "updated_at": (issue.updated_at.replace(tzinfo=UTC) if issue.updated_at else None),
                "closed_at": (issue.closed_at.replace(tzinfo=UTC) if issue.closed_at else None),
...
class GithubConnector(CheckpointedConnectorWithPermSyncGH[GithubConnectorCheckpoint]):
    ...
    def _fetch_from_github(
        ...
        if self.include_prs and checkpoint.stage == GithubConnectorStage.PRS:
            ...
            for pr in pr_batch:
                ...
                if start is not None and pr.updated_at and pr.updated_at.replace(tzinfo=UTC) <= start:
                    done_with_prs = True
                    break
                # Skip PRs updated after the end date
                if end is not None and pr.updated_at and pr.updated_at.replace(tzinfo=UTC) > end:
                    continue
                try:
        ...
        if self.include_issues and checkpoint.stage == GithubConnectorStage.ISSUES:
            ...
            for issue in issue_batch:
                ...
                # we iterate backwards in time, so at this point we stop processing prs
                if start is not None and issue.updated_at.replace(tzinfo=UTC) <= start:
                    done_with_issues = True
                    break
                # Skip PRs updated after the end date
                if end is not None and issue.updated_at.replace(tzinfo=UTC) > end:
                    continue
    ...
    def _load_from_checkpoint(
        ...
        end_datetime = datetime.fromtimestamp(end, tz=UTC) + ONE_DAY
    ...
    def retrieve_slim_document(
        ...
        end_value = datetime.now(UTC).timestamp() if end is None else end
