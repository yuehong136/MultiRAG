...
class ESConnectionBase(DocStoreConnection):
    def __init__(self, mapping_file_name: str="es_mapping.json", logger_name: str='multirag.es_conn'):
        ...
        fp_mapping = os.path.join(get_project_base_directory(), "configs", mapping_file_name)
    ...
    def create_idx(self, index_name: str, dataset_id: str, vector_size: int, parser_id: str = None):
        # parser_id is used by Infinity/Milvus but not needed for ES (kept for interface compatibility)
    ...
    def create_doc_meta_idx(self, index_name: str):
        """Create a per-tenant document metadata index."""
        if self.index_exist(index_name, ""):
            return True
        try:
            fp_mapping = os.path.join(get_project_base_directory(), "configs", "doc_meta_es_mapping.json")
            if not os.path.exists(fp_mapping):
