...
def _clean_headers(headers: dict[str, str] | None, auth_token: str | None = None) -> dict[str, str] | None:
        ...
        for attempt in range(retries + 1):
            try:
                ...
                if not _is_sensitive_url(url):
                    ...
                    logger.debug(f"async_request {method} {_redact_sensitive_url_params(url)} -> {response.status_code} in {duration:.3f}s")
                return response
    ...
    with httpx.Client(
        ...
        for attempt in range(retries + 1):
            try:
                ...
                duration = time.monotonic() - start
                if not _is_sensitive_url(url):
                    logger.debug(f"sync_request {method} {_redact_sensitive_url_params(url)} -> {response.status_code} in {duration:.3f}s")
                return response
            except httpx.RequestError as exc:
                last_exc = exc
                if attempt >= retries:
