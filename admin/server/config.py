    ...
    for k, v in raw_configs.items():
        match k:
            ...
            case "mysql":
                ...
                password = v.get("password", "")
                config = MySQLConfig(
                    id=id_count, name=name, host=host, port=port, username=username, password=password, service_type="metadata", meta_type="mysql", detail_func_name="get_database_status"
                )
                configurations.append(config)
                id_count += 1
