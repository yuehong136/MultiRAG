    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if content_length:
            ...
            if content_length > self.max_content_length:
                raise HTTPException(status_code=413, detail=f"Request body too large. Maximum size: {self.max_content_length} bytes")
...
def create_app() -> FastAPI:
    """创建 FastAPI 应用实例"""
    app = FastAPI(title="RAGFlow Admin API", description="RAGFlow 管理后台 API", version="1.0.0", lifespan=lifespan)
