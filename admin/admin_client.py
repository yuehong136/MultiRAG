...
class AdminCLI:
    ...
    def verify_admin(self, args):
        ...
        while True:
            ...
            try:
                # HTTP Basic Auth 会自动处理 base64 编码，不需要手动编码
                self.admin_password = admin_passwd
    ...
    def run_interactive(self):
        ...
        print("MultiRAG Admin command line interface - Type '\\?' for help, '\\q' to quit")
    ...
    def _handle_drop_user(self, command):
        username_tree: Tree = command['username']
        username: str = username_tree.children[0].strip("'\"")
        print(f"Dropping user: {username}")
        ...
        if response.status_code == 200:
            ...
            if res_json.get('code') == 0:
                ...
                print(f"Failed to delete user: {res_json.get('message')}")
        else:
            print(f"Request failed with status code: {response.status_code}")
